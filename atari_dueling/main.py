...
if __name__=='__main__':
    ...
    complete_name = '-algo ' + args.algo + ' -n_games ' + str(args.n_games) + ' -lr ' + str(args.lr) + ' -gamma ' + str(args.gamma) \
                    + ' -epsilon ' + str(args.epsilon) + ' -epsilon_min ' + str(args.epsilon_min) + ' -epsilon_dec ' + str(args.epsilon_dec)\
                    + ' -memory_size ' + str(args.memory_size) + ' -batch_size ' + str(args.batch_size) + ' -replace ' + str(args.replace)

    env_name_path = os.path.join(os.getcwd(), args.env_name)
    agent_path = os.path.join(env_name_path, complete_name)
    ...
    for i in range(n_games):
        ...
        if load_checkpoint:  # if we're testing, exit after 10 games
            if i == 10:
