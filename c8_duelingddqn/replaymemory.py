...
class ReplayMemory:
    def __init__(self, size, input_shape, n_actions):
        ...
        self.mem_counter = 0
        self.state_memory = np.zeros((self.size, *input_shape), dtype=np.float32)  # float32 is sufficient
        self.new_state_memory = np.zeros((self.size, *input_shape), dtype=np.float32)
        ...
        self.terminal_memory = np.zeros(self.size, dtype=np.uint8)  # later will be used as a mask to set everything to 0 for terminal states
